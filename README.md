...
- 四個 plugin：[`skill-tools`](plugins/skill-tools/README.md)、[`ponytail`](plugins/ponytail/README.md)、[`media-tools`](plugins/media-tools/README.md)、`taiwan-biz-research`
...
/plugin install taiwan-biz-research@how-skills
```
...
### 📊 taiwan-biz-research — 台灣商業案研究（資料層＋方法層）

三層架構，從資料到建議：

| Skill | 用途 |
|---|---|
| `tw-data` | 台灣經濟資料層：景氣燈號（1984-）、家庭收支（1976-）、上市月營收、外銷訂單、總經、**營業稅行業別銷售額（含中小企業）**——全部免費免 key 政府開放資料，政府端點的坑（ROC 年、破損憑證、200-非資料、靜默回退）都包掉了 |
| `strategy-frameworks` | 決策導向框架路由：什麼問題用什麼框架、用哪個資料指令餵它、以及**哪些框架在台灣的免費資料下根本做不出來**（做不出來本身就是發現） |
| `biz-case-workflow` | 多 agent 端到端工作流：界定決策 → 抓數 → 交叉驗證 → 三視角對抗性駁斥 → 合成建議。約 15-25 隻 agent，給需要「經得起攻擊」而非「聽起來合理」的結論用 |

場外來源（DMA 廣告量、Reuters DNR 付費率、TAICCA 產值）已驗證取法並快照於
`tw-data/references/external-sources.md`。純 Python 標準庫，無額外依賴。
...
how-skills/
├── .claude-plugin/
│   └── marketplace.json                # marketplace 目錄，列出所有 plugin
├── plugins/
│   ├── skill-tools/
